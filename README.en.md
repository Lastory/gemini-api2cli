...
| Method | Path                 | Description                                                  |
| ------ | -------------------- | ------------------------------------------------------------ |
| GET    | `/v1/models`         | List available models (compatible with OpenAI `data` format) |
| GET    | `/v1/models/:model`  | Retrieve single model details (OpenAI format)                |
...
Standard Base URL is `http://localhost:41242/v1` (e.g. for SillyTavern,
NextChat, or OpenAI SDKs).

| Method | Path                   | Description                                                  |
| ------ | ---------------------- | ------------------------------------------------------------ |
| POST   | `/v1/chat/completions` | Chat Completions standard endpoint (supports `stream: true`) |
| GET    | `/v1/models`           | List models (standard OpenAI format with `data: [...]`)      |
| GET    | `/v1/models/:model`    | Retrieve model details (standard OpenAI format)              |
...
### Third-Party Client Setup (e.g. SillyTavern, NextChat)

- **Using OpenAI-Compatible Mode (Recommended)**:
  - **API**: `Chat Completion`
  - **Source / Protocol**: `Custom (OpenAI-compatible)` or `OpenAI`
  - **Custom Endpoint / Reverse Proxy**: `http://127.0.0.1:41242/v1`
  - **API Key**: Your configured Token (default is `root`)
  - Click **Connect**: The status indicator will turn green ("Valid") and
    available models will be loaded automatically into the dropdown.
- **Using Google AI Studio Mode**:
  - **Source / Protocol**: `Google AI Studio`
  - **Reverse Proxy**: `http://127.0.0.1:41242`
  - **API Key**: Your configured Token (default is `root`)
...
SillyTavern / API Client
    │
    ▼
prompt-api layer (/v1/chat/completions, /v1beta/models/*, /v1/gemini/*)
    │
    ▼
